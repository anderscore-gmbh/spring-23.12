...
public class TaskControllerTest {
    ...
    public void deleteNotExisting() throws Exception {
        ResultActions resultActions = mockMvc.perform(delete("/tasks/42"))
                .andExpect(status().isNotFound());
        checkErrorContent(resultActions, 404, "There is no");
    }

    private void findNotExisting() throws Exception {
        ResultActions resultActions = mockMvc.perform(get("/tasks/42"))
                .andExpect(status().isNotFound());
        checkErrorContent(resultActions, 404, "There is no");
    }
    ...
    private void updateNotExisting() throws Exception {
        ...
        ResultActions resultActions = mockMvc.perform(put("/tasks/42").content(task).contentType(MediaType.APPLICATION_JSON_UTF8))
                .andExpect(status().isNotFound());
        checkErrorContent(resultActions, 404, "There is no");
    }
